"use client"
import { Button, Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Popover, PopoverContent, PopoverTrigger, useDisclosure} from '@nextui-org/react';
...
function Neurodivergencias() {
  const [neurodivergencias, setNeurodivergencias] = useState<Neurodivergencia[]>([]);
  const [nombre, setNombre] = useState("");
  const [neurodivergenciaSeleccionada, setNeurodivergenciaSeleccionada] = useState<Neurodivergencia | null>(null);
  const { onOpen: onEditOpen, onOpenChange: onEditOpenChange, isOpen: isEditOpen } = useDisclosure();
  ...
  const eliminar = async (onClose: () => void) => {
    ...
    try {
      const response = await axios.delete(`/api/NEE/${neurodivergenciaSeleccionada.id}`);
      if (response.status >= 200 && response.status < 300) {
  ...
  const modificar = async () => {
    ...
    try {
      const response = await axios.put(`/api/NEE/${neurodivergenciaSeleccionada.id}`, data);
      if(response.status >= 200 && response.status < 300) {
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
            <ModalContent>
              {(onClose) => (
                <>
                  <ModalHeader>
                    ¿Seguro que desea eliminar la neurodivergencia: {neurodivergenciaSeleccionada?.nombre}?
                  </ModalHeader>
                  <ModalFooter>
          ...
          <ul>
            {neurodivergencias.length === 0 ? (
              ...
              neurodivergencias.map((neurodivergencia, index) => (
                <li key={index}> {neurodivergencia.nombre}  
                <button onClick={() => handleEliminar(neurodivergencia)}> eliminar </button>
                <button onClick={() => handleModificar(neurodivergencia)}> Modificar </button>
                </li>
              ))
            )}
          </ul>
