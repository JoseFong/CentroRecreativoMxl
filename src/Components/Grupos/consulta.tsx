...
import {Button, Card, CardBody, Spinner, Tab, Tabs, Tooltip, useDisclosure} from "@nextui-org/react";
...
import {FaCalendarDays, FaPrint} from "react-icons/fa6";
    ...
    const fetchSalidas = async () => {
        try {
            ...
            if (response.status >= 200 && response.status < 300) {
                ...
                fechaLimite.setDate(fechaHoy.getDate()-1);

                const filteredSalidas = response.data.filter((salida: any) => {
                    const fechaSalida = new Date(salida.fecha);
                    return fechaSalida >= fechaLimite;
                });

                setSalidas(filteredSalidas);
            } else {
                throw new Error(response.data.message || "Error desconocido.");
            }
        } catch (e: any) {
                                                ...
                                                <CardBody>
                                                    ...
                                                    <div className="overflow-y-auto max-h-[32rem]">
                                                        <Table aria-label="Example static collection table">
                                                            <TableHeader>
                                                                <TableColumn
                                                                    className="bg-headerNav text-[#ffffff] text-sm md:text-md font-bold">
                                                                    Nombre(s)
                                                                </TableColumn>
                                                                <TableColumn
                                                                    className="bg-headerNav text-[#ffffff] text-sm md:text-md font-bold">
